...
emergencia('perdida me motor','bomberos').
emergencia('parto','medicos').
emergencia('paro cardiaco' ,'medicos').
emergencia('secuetro','policia').

%Bloque de listas con los mensajes admitidos por
%el usuario para asi la torre pueda responder
saludos(['hola', 'buenas', 'buenos dias','buenas tardes','buenas noches'], "-MayCEy: Hola ¿en qué lo puedo ayudar?").

despedidas(['Adios','Hasta luego','Cambio y Fuera','adios','hasta luego','cambio y fuera'],"-MayCEy: Hasta luego").

agradecimientos(['Gracias', 'Muchas gracias'], "-MayCEy: Con mucho gusto").
agradecimientos(['gracias', 'muchas gracias'], "-MayCEy: Con mucho gusto").
...
solicitud(M,Agenda):-
     ...
     miembro(M,ListaSaludos),
     write(Resp),
     torre_libre(Agenda).

%Solicitud de inicio de la torre para despegar.
solicitud(M,Agenda):-
    identificar(M,'despegar'),
    buscar_pista_despegue(_,_,_,_,Vuelo,Agenda),
    torre_libre([Vuelo|Agenda]). %La respuesta retorna la funcion Agenda para que se guarde

%solicitud para aterrizar,
solicitud(N,Agenda):-
    identificar(N,'aterrizar'),
    buscar_pista_aterrizaje(_,_,_,_,_,Vuelo,Agenda),%llama a la funcion que busca una pista libre
...
solicitud(N,Agenda_previa):-
    identificar(N,'mayday'),
    buscar_pista_emergencia(_,_,_,Agenda_previa,Agenda),%llama a la funcion de emergencias
    torre_libre(Agenda).

solicitud(N,Agenda_previa):-
    identificar(N,'emergencia'),
    buscar_pista_emergencia(_,_,_,Agenda_previa,Agenda),%llama a la funcion de emergencias
    torre_libre(Agenda).
...
%solicitud de despedida
solicitud(N,Agenda):-
    despedidas(ListaDeDespedidas,Resp),
    miembro(N,ListaDeDespedidas), % al terminar el turno, guarda la agenda en memoria
    write(Resp),
    torre_libre(Agenda).

%solicitud de agradecimiento
solicitud(N,Agenda):-
    identificar(N,'gracias'),
    agradecimientos(ListaDeAgradecimientos,Resp),
    miembro(N,ListaDeAgradecimientos),
    write(Resp),
    torre_libre(Agenda).

solicitud(_,Agenda):-
     write("-MayCEy: Por Favor, repita su mensaje"),
     torre_libre(Agenda).
...
torre_libre(Agenda):-
    ...
    write('-Usuario: '),
    read(X),
    solicitud(X,Agenda). %lee la solicitud y la envia
...
buscar_pista_emergencia(Id,Avion,Emergencia,Agenda_previa,Agenda):-
    write("-MayCEy: Por favor identifiquese"),read(Id),  % se lee la matricula
    write("-MayCEy: Indique cual es su emergecnia"),read(Emergencia), % se lee la emergencia
    write("-MayCEy: Cual es la Aeronave?"),read(Avion), % se lee el avion.
    ...
    write("-MayCEy: La pista "),write(P),write(" ya se encuentra despejada y un equipo de "),
...
buscar_pista_despegue(Id,Aeronave,Hora,Direccion,Vuelo,Agenda):-
    write("-MayCEy: Por favor identifiquese"),read(Id), % se lee la matricula
    write("-MayCEy: Indique la aeronave"),read(Aeronave), % se lee el avion.
    write("-MayCEy: Indique su hora de salida"),read(Hora), % se lee la hora
    write("-MayCEy: Indique su Direccion"),read(Direccion),% se lee la direccion de vuelo
    ...
    write("-MayCEy: Su pista es: "),write(PistaOficial), % le indica la pista de despegue
...
buscar_pista_aterrizaje(Id,Aeronave,Velocidad,Distancia,Direccion,Vuelo,Agenda):-
   write("-MayCEy: Por favor identifiquese"),read(Id),
   write("-MayCEy: indique la aeronave"),read(Aeronave),
   write("-MayCEy: indique su velocidad"),read(Velocidad),
   write("-MayCEy: Confirme su distancia hasta el aeropuerto"),read(Distancia),
   write("-MayCEy: Por ultimo la direccion del vuelo"),read(Direccion),
   ...
   write("-MayCEy: Su pista asignada es la: "),write(PistaOficial),write("por favor "),write(V),
...
%Funcion que determina si hay un codigo 7500
velocidadYhora(Hora_llegada,Velocidad,Distancia,_,Indicacion_velocidad):-
    (Distancia=7500 ; Velocidad=7500),
    Indicacion_velocidad = incremente,
    Hora_llegada = "05 lo antes posible \n equipo de policia enviado".
...
identificar(Texto,Palabra):-
	findall(B,sub_atom(Texto,_,_,_,B),ListaPalabras),
